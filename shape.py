...
import numpy as np
from dtaidistance import dtw,ed,dtw_ndim
import bisect
import math
...
class finder():
    ...
    def find_patterns_opposite(self, metric='euclidean', min_d=0.5, dtw_sel=0, select=True, dis_sel=5):
        """
        Finds custom patterns in the given dataset using the interactive shape finder.
        ...
        if dis_sel==0:
            tot = tot.sort_values([1])
            tot = tot[(tot[1]<0.75) & (tot[1]>0.5)]
            toti = tot[0]
        elif dis_sel==1:
            tot = tot.sort_values([1])
            tot = tot[(tot[1]<1) & (tot[1]>0.75)]
            toti = tot[0]
        elif dis_sel==2:
            tot = tot.sort_values([1])
            tot = tot[(tot[1]<1.25) & (tot[1]>1)]
            toti = tot[0]
        elif dis_sel==3:
            tot = tot.sort_values([1])
            tot = tot[(tot[1]<1.5) & (tot[1]>1.25)]
            toti = tot[0]
        elif dis_sel==4:
            tot = tot.sort_values([1])
            tot = tot[tot[1]>1.5]
            ...
            tot = tot.sort_values([1],ascending=False)
            len_tot = len(tot[tot[1] < min_d])
            tot = tot.iloc[:len_tot,:]
            toti = tot[0]
        ...
        for i in range(len(n_test)):
            ...
            if i not in exclude:
                ...
                if seq2.var() != 0.0:
                    seq2 = (seq2 - seq2.min()) / (seq2.max() - seq2.min())
                    try:
                        tot.append(ed.distance(seq1, seq2))
                    except:
                        # Ignore any exceptions (e.g., divide by zero)
                        pass
        return tot
